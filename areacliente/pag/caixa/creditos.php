<?php
		...
		CASE Tipo WHEN '1' THEN IF(Entrada != 0.00, Entrada, (IF(VParcela != 0, VParcela, Valor))) ELSE '0.00' END AS Entr FROM caixa WHERE Tipo = 1
		UNION
		...
		WHERE Tipo = 1 ORDER BY Data
		";
		
		
		$Bal->Tabs = array('ID','Status','Data','Finalidade','Categoria','Valor','Parcelas', 'IDParcela', 'VParcela', 'Entrada','Tipo','IdCl','Obs','Entr');
		$Bal->MontaSQLRelat();
		$Bal->Load();
...
<h1>Relatório de Créditos</h1>
		...
		$t->Cols = array('item','Data','Status','Cliente','Finalidade','Entr','action','ID');
		...
		';
		$t->Query = $Bal->Query;
		$t->setTitulos(array('#:center','Data:center','Origem:center','Cliente:left','Finalidade:left','Entrada:ceter','Ações:center','ID:center'));
		$t->showTab();
 		?>