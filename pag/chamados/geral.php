...
if($VerMais == 0) :
		...
		//$W->AddAviso($UsT->Clientes('ch.IdCl'));
		...
		WHERE ch.Status > '3' AND ch.Status != '14'".((!empty($B->Where)) ? " AND ".$B->Where : "")."".((!empty($UsT->Clientes('ch.IdCl'))) ? " AND ".$UsT->Clientes('ch.IdCl') : "")." ORDER BY ch.Status DESC";
		$tt2->MontaSQLRelat();
		$tt2->Load();
...
<h1>Consultar Chamados</h1>
		...
		<div class="fundoTabela">
                ...
                <?php
						...
						FROM chamado AS ch INNER JOIN statuschamados AS st ON st.ID = ch.Status INNER JOIN clientes AS cl ON cl.ID = ch.IdCl INNER JOIN equipamento AS eq ON eq.ID = ch.IDEquipamento WHERE ch.Status > '0' AND ch.Status < '4' AND ch.Status != '14'".((!empty($B->Where)) ? " AND ".$B->Where : "")."".((!empty($UsT->Clientes('ch.IdCl'))) ? " AND ".$UsT->Clientes('ch.IdCl') : "")." ORDER BY ch.Status DESC";
						$tt->MontaSQLRelat();
						$tt->Load();
				?>
